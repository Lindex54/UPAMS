...
@section('title', 'Add User | Property Management')
    ...
    <div class="mx-auto flex max-w-3xl flex-col gap-6">
        <section class="rounded-xl border border-border bg-white p-6 shadow-sm sm:p-8">
            <div>
                ...
                <h2 class="mt-2 text-2xl font-semibold text-heading">Create a user account</h2>
                <p class="mt-2 text-sm leading-6 text-body-text">Enter the user’s basic information and set their initial sign-in password.</p>
            </div>
            ...
            <form class="mt-8 flex flex-col gap-8" method="POST" action="{{ route('users.store') }}">
                @csrf

                @include('users._form', ['user' => null, 'includePassword' => true])
                ...
                <div class="flex flex-wrap items-center justify-end gap-3 border-t border-border pt-6">
                    ...
                    <button class="inline-flex min-h-11 items-center rounded-lg bg-busitema-gold px-5 text-sm font-semibold text-busitema-navy shadow-sm transition hover:bg-busitema-yellow focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-busitema-blue" type="submit">Create user</button>
                </div>
            </form>
        </section>
    </div>
